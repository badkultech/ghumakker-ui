...
import { Send, AlertCircle, MessageCircle, Check } from "lucide-react";
...
export default function DesktopSidebar({
  ...
  return (
      ...
      <div className="space-y-4">
        ...
        {bookedTripDetail ? (
          <div className="bg-white rounded-2xl border p-6 space-y-6 shadow-sm">
            {/* Green Success Box */}
            <div className="bg-[#E2F5D3] rounded-2xl p-6 text-center space-y-2">
              <div className="text-2xl mb-1">🎉</div>
              <h3 className="text-xl font-bold text-gray-900 leading-tight">You're In!</h3>
              <p className="text-sm text-gray-600 font-medium">Booking confirmed for this trip</p>
            </div>

            {/* Reference info */}
            <div className="text-center space-y-1">
              <p className="text-sm text-gray-400 font-medium">Booking Reference</p>
              <p className="text-2xl font-black text-[#3EB008] tracking-tight">
                #{bookedTripDetail.bookingRef || bookedTripDetail.id}
              </p>
            </div>

            {/* Action Buttons */}
            <div className="space-y-3">
              <Link href={`/home/booking-confirmation/${bookedTripDetail.bookingRef || bookedTripDetail.id}`} className="block">
                <button className="w-full py-4 bg-[#3EB008] text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:opacity-95 transition-all active:scale-[0.98] cursor-pointer shadow-md shadow-green-200">
              ...
              <button 
                onClick={onAsk}
                className="w-full py-3.5 bg-white border border-primary/30 text-primary rounded-full font-bold flex items-center justify-center gap-2 hover:bg-primary/5 transition-all active:scale-[0.98] cursor-pointer"
              >
              ...
              <>
                {dynamic?.pricingCategoryDtos?.map((cat: any, i: number) => {
                  ...
                  if (cat.pricingCategoryType === "SINGLE") {
                    ...
                    return (
                      <div key={i} className="bg-gray-50 rounded-xl p-4 space-y-2 border border-green-200">
                        <p className="font-medium text-sm flex justify-between">
                          {cat.categoryName}
                          <span className="text-primary text-xs">Included automatically</span>
                        </p>
                  ...
                  return (
                    <div key={i} className="bg-gray-50 rounded-xl p-4 space-y-3">
                      ...
                      {cat.pricingCategoryOptionDTOs?.map((opt: any, idx: number) => {
                        const checked = selectedOptions[cat.categoryName]?.name === opt.name;
    ...
    `}
      ...
      flex justify-between items-center p-3 rounded-xl border cursor-pointer
      transition-all
      ${selectedAddOns.includes(add.name) ? "border-orange-400 bg-orange-50" : "hover:bg-gray-50"}
    `}
                      >
                        <div className="flex items-start gap-3">
                          {/* CUSTOM CHECKBOX */}
                        ...
                        <p className="font-semibold text-sm">₹{(add.charge * numTravelers).toLocaleString()}</p>
                      </label>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                className={`w-full flex items-center justify-center gap-3 py-4 rounded-2xl font-bold text-lg cursor-pointer shadow-lg transition-all active:scale-[0.98] ${
                  isButtonEnabled ? "bg-brand-gradient text-white hover:opacity-90" : "bg-gray-200 text-gray-400 cursor-not-allowed"
                }`}
              >
                {!isSoldOut && (
